...
//                 if (cbComputers.Checked) selectedInterests += "Комп'ютери, ";
//                 if (cbSports.Checked) selectedInterests += "Спорт, ";
//                 if (cbArt.Checked) selectedInterests += "Мистецтво, ";
//                 if (cbScience.Checked) selectedInterests += "Наука, ";

//                 if (selectedInterests.Length > 0)
//                     selectedInterests = selectedInterests.Substring(0, selectedInterests.Length - 2);

//                 string message = $"Ім'я: {txtName.Text}\nВік: {cmbAge.Text}\nСтать: {(rbMale.Checked ? "Чоловіча" : "Жіноча")}\nВаші інтереси: {selectedInterests}\nВаш коментар: {txtOpinion.Text}";
//                 MessageBox.Show(message, "Вибрані параметри", MessageBoxButtons.OK, MessageBoxIcon.Information);
//             };
